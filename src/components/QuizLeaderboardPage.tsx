...
import { ArrowLeft, Trophy, Medal, Award, Clock, Users, Target, Star, Crown } from 'lucide-react';
...
const QuizLeaderboardPage: React.FC<QuizLeaderboardPageProps> = ({ quiz, onBack }) => {
  ...
  const averageTime = Math.round(leaderboard.reduce((sum, entry) => sum + entry.timeTaken, 0) / leaderboard.length);
        ...
        <div className="relative">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            ...
            <div className="bg-white bg-opacity-20 rounded-lg p-3">
              ...
              <div className="text-lg font-bold capitalize">{quiz.status}</div>
            </div>
          </div>
        </div>
      </div>
            ...
            <div className="text-center">
              <div className="relative mb-3">
                <img
                  ...
                  className="w-16 h-16 rounded-full object-cover mx-auto border-4 border-amber-400"
                />
                <div className="absolute -top-2 -right-2 w-8 h-8 bg-amber-500 rounded-full flex items-center justify-center">
                  <span className="text-white font-bold text-sm">3</span>
                </div>
              </div>
              <div className="bg-amber-100 rounded-lg p-3 h-20 flex flex-col justify-end">
                <h3 className="font-semibold text-gray-900 text-sm">{leaderboard[2].name}</h3>
                <p className="text-amber-700 text-xs">{leaderboard[2].score}%</p>
              </div>
            </div>
          )}
        </div>
          ...
          <table className="w-full">
            ...
            <tbody className="divide-y divide-gray-200">
              {leaderboard.map((entry) => (
                <tr key={entry.id} className={`hover:bg-gray-50 ${entry.rank <= 3 ? getRankBg(entry.rank) + ' bg-opacity-10' : ''}`}>
                  ...
                  <td className="py-4 px-6">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-500 text-sm">
                        ({entry.score * entry.totalQuestions / 100}/{entry.totalQuestions})
                      </span>
                    </div>
                  </td>
                  <td className="py-4 px-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">{leaderboard[0]?.score}%</p>
            </div>
          </div>
          <p className="text-gray-600 text-sm">Achieved by {leaderboard[0]?.name}</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              <h3 className="font-semibold text-gray-900">Fastest Time</h3>
              <p className="text-2xl font-bold text-blue-600">
                {Math.min(...leaderboard.map(e => e.timeTaken))}m
              </p>
            </div>
          </div>
          <p className="text-gray-600 text-sm">
            By {leaderboard.find(e => e.timeTaken === Math.min(...leaderboard.map(e => e.timeTaken)))?.name}
          </p>
        </div>
          ...
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-2xl font-bold text-purple-600">
                {Math.round((quiz.participants / quiz.registrations) * 100)}%
              </p>
            </div>
          </div>
          <p className="text-gray-600 text-sm">
            {quiz.participants} of {quiz.registrations} registered
          </p>
        </div>
      </div>
    </div>
